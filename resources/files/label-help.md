These documentations are a WIP and are currently only available in English.

# Expressions

Expressions are special tests that can be used to test if a value is something
Examples:

`"{ noclip_deaths > 0 ? "Noclip deaths is higher than 0" : "Noclip deaths is 0" }"`
`"{ level_likes > 0 ? "People like this level" : "People dont like this level" }"`
...
**player1_max_cps**  |  The highest clicks per second for player 1 for this attempt
**player2_max_cps**  |  The highest clicks per second for player 2 for this attempt
**max_cps**  |  The highest clicks per second for both players for this attempt
